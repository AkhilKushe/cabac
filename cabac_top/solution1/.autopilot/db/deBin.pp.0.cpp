...
void parseSAOTypeIdx(arith_t& state, UChar* bStream, UChar ctxTables[512], UInt& symbolVal){
 symbolVal = 0;
 bool binVal;
 decode_decision(REGULAR, state, binVal, bStream, 1, ctxTables);
 if(!binVal) {
  symbolVal = binVal;
  return;
 } else {
  decode_decision(BYPASS, state, binVal, bStream, 1, ctxTables);
  if(binVal) {
   symbolVal = 2;
  } else {
   symbolVal = 1;
...
void parseSAOEO(arith_t& state, UChar* bStream, UInt& symbolVal){
 ...
 VITIS_LOOP_80_1: while(binIdx < 2) {
  decode_decision(BYPASS, state, binVal, bStream, 0, 0);
...
void parseSAOBO(arith_t& state, UChar* bStream, UInt& symbolVal){
 ...
 VITIS_LOOP_96_1: while(binIdx < 5) {
  decode_decision(BYPASS, state, binVal, bStream, 0, 0);
  symbolVal = (symbolVal << 1) + binVal;
  binIdx +=1;
 }
...
void sao_top(data_in_t& inp,data_out_t& out, arith_t& state, UChar* bStream, UChar ctxTables[512]){
 ...
 if( (!sao_merge_up_flag) && (!sao_merge_left_flag)) {
  VITIS_LOOP_142_1: for(int clIdx=0; clIdx < 3; clIdx++) {
   ...
   VITIS_LOOP_189_4: for(int i=0; i<4; i++){
  ...
  if (sao_merge_left_flag) {
   VITIS_LOOP_201_5: for(int i=0; i<3; i++){
  ...
  if (sao_merge_up_flag) {
   VITIS_LOOP_213_6: for(int i=0; i<3; i++){
