...
__attribute__((sdx_kernel("cabac_top", 0))) void cabac_top(volatile UChar globalCtx[512], hls::stream<UChar>& bitStream, hls::stream<UInt>& bitOut, initData_t initd);
# 2 "src/top.cpp" 2
...
void decode_decision(bool init, BAEMode mode, arith_t& state, bool& binVal, UChar* bStream, UInt ctxAddr, UChar ctxTables[512]);
# 5 "src/top.cpp" 2

void load_ram(UChar ctxTable[512], hls::stream<UChar>& inpStream, UInt cnt){
 VITIS_LOOP_7_1: for(int i=0; i < cnt; i++) {
  ctxTable[i] = inpStream.read();
 }
}

void store_global_ram(volatile UChar globalCtx[512], UChar ctxTable[512], UInt cnt){
 VITIS_LOOP_13_1: for(int i=0; i < cnt; i++) {
...
__attribute__((sdx_kernel("cabac_top", 0))) void cabac_top(volatile UChar globalCtx[512], hls::stream<UChar>& bitStream, hls::stream<UInt>& bitOut, initData_t initd){_ssdm_SpecArrayDimSize(globalCtx, 512);
...
#pragma HLS STREAM depth=512 type=fifo variable=streamCtxRAM
 ...
 UChar tempBst[4];
 VITIS_LOOP_40_1: for(int i=0; i<4; i++){
